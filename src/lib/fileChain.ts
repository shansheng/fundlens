...
/**
 * 移动端保存文件（备份导出 .db）：优先 Web Share 携带文件（系统分享到文件管理/网盘），
 * 不支持时退化为 <a download>（依赖 WebView 下载监听，成功与否由调用方提示兜底）。
 */
export async function shareFileMobile(fileName: string, mime: string, b64: string): Promise<boolean> {
  ...
  // 兜底：触发 WebView 下载（Tauri Android 若无 DownloadListener 可能静默失败，提示用户）
  try {
